...
# Results layout
st.markdown(f'<div class="card" style="color:{TEXT_COLOR};">', unsafe_allow_html=True)
colA, colB = st.columns([1,1], gap="medium")  # proper spacing
with colA:
    st.image(display_img, caption="Uploaded Cell Image", width=IMAGE_DISPLAY_WIDTH)
    st.write("")
    st.write("**Prediction**")
    st.markdown(f"<div style='font-size:18px; font-weight:700; color:{BRAND_COLOR}; margin-bottom:10px;'>{predicted_label.upper()}</div>", unsafe_allow_html=True)
